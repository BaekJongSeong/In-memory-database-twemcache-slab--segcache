...
seg_data_start(int32_t seg_id)
{
    return heap.base + heap.seg_size * seg_id;
}


static void check_seg(void){
    int cc_count = 0;uint32_t ait_count=0; uint32_t erase_count=0;
 int seg_count=0; uint64_t total_bytes=0;uint64_t wasted=0; int sum=0; int sum_count=0;
rstatus_i   status;
struct seg  *seg;
int32_t     seg_id, next_seg_id;
    ...
    for (int i = 0; i < MAX_N_TTL_BUCKET; i++) {
        ...
        while (1) {
            ...
            uint8_t seg_data = seg_data_start(seg_id);
            //segment안의 모든 item에 대해서 total 값들 뽑고
            //if len이 크면 무시해야지
            //printf("seg address is %p", &seg_data);
            uint8_t * seg_temp = &seg_data;
            size_t sz=0;
            
            for (int i = 0; i< 1048576; i+=sz){
                struct item* it = (struct item *)seg_temp;
                if (! item_size(it->klen,it->vlen,it->olen) > 0)break;
                if ( item_size(it->klen,it->vlen,it->olen) > 1048576)continue;
                sz = item_size(it->klen,it->vlen,it->olen);
                sum+=sz;
                seg_temp+=sz;
                //printf("item size %d add %p\n", sz, seg_temp);
                sum_count+=1;
            }
            ait_count+=sum_count; total_bytes+=sum;
            
            if (seg->create_at + seg->ttl < time_proc_sec() - 2 ||
            seg->create_at < flush_at) {//segment 안에 있는 모든 item이 expired구만 그만큼 ++
            cc_count+=sum_count; wasted+=sum;
            }
...
FILE* fp3;
fp3 = fopen("./1000_log/Seg_print_log.txt", "a");
use_slab = fopen("./1000_log/Seg_600_1205_log_nsbin.txt","a"); 
...
//            printf("curr sec %d\n", time_proc_sec());
        ...
        if(n_req % 500000 == 0){check_seg();//check_slab2();
        }
    ...
    duration_stop(&d);
    fprintf(fp3,"this is log for Seg 600 1205 final seg program n.sbin\n"); //2000, 4000
