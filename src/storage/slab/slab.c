...
pthread_t            bg_tid;
//static 
extern struct slab_heapinfo heapinfo;             /* info of all allocated slabs */
 ...
 */
static void
_slab_lruq_rebuild(const uint8_t *heap_start)  //user data pool 위치가 바뀌어버리면 slab 참조 위치 바뀌므로 오케이ㅋㅋ
{
...
printf("the slab size is %ld\n",slab_size);
printf("create slab table with %ld entries\n", heapinfo.max_nslab);

if(heapinfo.curr != NULL){
uint8_t* heap_start=heapinfo.curr;
for(int i=0; i< heapinfo.max_nslab;i++){
struct slab *slab = (struct slab* )heap_start;
printf("what is prealloc?? heapinfo.curr %p, slab initialized: %d\n",&(*heap_start),slab->initialized);heap_start+=slab_size;}
...
_slab_profile_setup(char *profile_str)
{
   ...
   printf("setup slab profile with: %u number index\n", profile_last_id);
   printf("slab profile num %u:\n",profile_last_id);
...
slab_teardown(void)
{
    ...
    //fclose(fp2);
    //fclose(use_slab);
    fprintf(file_all,"real_set_count: %u ",real_set);
    fprintf(file_all,"real_evict_count: %u ",real_evict);
    fprintf(file_all,"real_delete_count: %u\n",real_delete);
    fclose(file_all);
...
static void
check_slab_expire(void)
{int cc_count = 0; size_t wasted=0;uint32_t ait_count=0;
    // slab_lruq를 지속적으로 돌면서
    struct slab* slab = TAILQ_FIRST(&heapinfo.slab_lruq);
//FILE* fp2 = fopen("./1000_log/1000_slab_expire_new.txt", "a");
if (slab!=NULL)//(slab->initialized==1)
{
    ...
    p = &slabclass[slab->id];
        ...
        {struct item* it = _slab_to_item(slab,idx,p->size);//slab_profile[slab->id]);
            if (it->is_linked==1)
            {ait_count+=1;
            if((it->expire_at < time_proc_sec() || it->create_at <= flush_at))
...
fprintf(fp2,"total nitem: %u ", ait_count);
fprintf(fp2,"%d %d\n", cc_count,wasted);
//fclose(fp2);
}
...
background_main(void *data)
{
    ...
    log_info("slab background_jongseong_made_ thread started");

    while (true) {
        check_slab_expire();
        //usleep(200); // 원래는 이게 200000. 그니까 0.2초로 되어있었어.
    }
...
{uint32_t cc_count = 0; uint64_t wasted=0;uint32_t ait_count=0;
int slab_count=0; uint64_t total_bytes=0;
    // slab_lruq를 지속적으로 돌면서
    struct slab* slab = TAILQ_FIRST(&heapinfo.slab_lruq);
if (slab!=NULL)//(slab->initialized==1)
{
     for (; slab != NULL; slab = TAILQ_NEXT(slab, s_tqe)) {
         ...
         if (slab_count > 4000) break;

    slab_count += 1;    int idx = 0;
    struct slabclass *p;
    p = &slabclass[slab->id];
...
fprintf(use_slab,"%u %llu\n", cc_count,wasted);
}


static void *
background_main_use_slab_check(void *data)
{
#ifdef __APPLE__
    pthread_setname_np("segBg");
#else
    ...
    log_info("slab background_1012_jongseong_made_ thread started");
...
void
start_background_thread_use_slab_check(void *arg)
{
    int ret = pthread_create(&bg_tid, NULL, background_main_use_slab_check, arg);
    if (ret != 0) {
        log_crit("pthread create failed for background thread: %s",
            strerror(ret));
        exit(EX_OSERR);
...
{
    //fp2 = fopen("./1000_log/log_slab_expire_new_1012.txt", "a"); //./1000_log/1000_slab_expire_new.txt
    //fp = fopen("./log/log_slab_num0_0930.txt", "a"); //./1000_log/1000_slab_num0_0930.txt
    //file_item = fopen("./1000_log/unique_item_num.txt", "w+");
    //use_slab = fopen("./1000_log/4000_final_log_1019_csbin.txt","a");   
    file_all = fopen("./1000_log/print_log2.txt","a");   
    real_set=0;real_delete=0;real_evict=0;
    ...
    printf("Slab header size: %d, item header size: %d\n", SLAB_HDR_SIZE,
            ITEM_HDR_SIZE);
...
//start_background_thread_use_slab_check(NULL);
...
fprintf(fp, "slab evict= slabclass: %d, slab locate: %p\n",slab_id(item_ntotal(it->klen, it->vlen, it->olen)), &(*slabslab));
//fclose(fp);
}*/
