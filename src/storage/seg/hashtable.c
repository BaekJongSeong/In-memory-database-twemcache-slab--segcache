...
{                       //uint64_t ==> 8byte, 1slot이 8byte이므로 * 8하면 64 => 그래서 N_BYTE_PER_BUCKET = 8
    uint64_t *table =
...
hashtable_setup(uint32_t hash_power)
{
    ...
    printf("create hash table of %ld entries %ld buckets, but what is hashmask? %ld\n",
        n_slot, n_slot >> N_SLOT_PER_BUCKET_LOG2,hash_table.hash_mask);
}
...
/*
FILE* fp = fopen("./log/llog.txt","a");
fprintf(fp,"hi ");
fprintf(fp, "bkt %ld: new_item_key %s, ", ((hv) & (hash_table.hash_mask)),key);
*/
...
//
//fprintf(fp,"new_item: %s || old_item: __ || same: %d",key,_same_item(key, klen, item_info));//item_key(_info_to_item(item_info))
//fprintf(fp,"\n");
//
...
//          
//int k=0;
//for (int j = 1; j < n_item_slot; j++)
//    if (bkt[j]!=0) k+=1;
//fprintf(fp,"the bucket has %d item. ",k);
//
...
FILE* fp = fopen("./log/llog.txt","a");
fprintf(fp,"hi ");
fprintf(fp, "bkt %ld: ", ((hv) & (hash_table.hash_mask)));
int k=0;
int flag=0;
for (int j = 1; j < 8; j++)
    if (bkt[j]!=0) k+=1;
fprintf(fp,"the bucket has %d item. the evict item key is %s\n",k,oit_key);
*/
...
/*
fprintf(fp,"\told_item: %s || same: %d",item_key(_info_to_item(item_info)),_same_item(oit_key, oit_klen, item_info));
fprintf(fp,"\n");
*/
            if (GET_TAG(item_info) != tag) {
                continue;
...
for (int j = 1; j < 8; j++)
    if (bkt[j]!=0) kk+=1;
fprintf(fp,"%d item evict. now the bucket has %d item. \n",k-kk,kk);
 */
                }
//fprintf(fp,"alright, you're here\n");
            } else {
