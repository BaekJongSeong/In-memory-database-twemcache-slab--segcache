...
//fprintf(fp,"SEG_HDR_SIZE: %ld, ITEM_HDR_SIZE: %ld", SEG_HDR_SIZE,ITEM_HDR_SIZE);
//fclose(fp);
...
seg_teardown(void)
{
    ...
    fprintf(file_all,"real_expire_count: %u\n",real_expire);
    fclose(file_all);
    stop = true;
...
seg_setup(seg_options_st *options, seg_metrics_st *metrics)
{
    ...
    file_all = fopen("./1000_log/Seg_print_log.txt","a"); 
    real_set=0;real_delete=0;real_evict=0;real_expire=0;
...
printf("watch!!\n n_thread: %d, seg_size: %ld heap_size(cache size): %ld\n",n_thread,heap.seg_size,heap.heap_size);
printf("heap_prealloc: %d, poolpath: %s\n poolname: %s\n",heap.prealloc,heap.poolpath,heap.poolname);
